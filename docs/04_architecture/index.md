...
## 2. 推奨技術スタック
マルチチャネル受付と証憑処理が中心。Backend は Kotlin/Spring Boot、文書/画像抽出は Python、ワークフローは Temporal、ケースDBは PostgreSQL、画像・PDFは S3、検索は OpenSearch。将来的な画像理解や fraud scoring のため Feature Store を別置きすると拡張しやすい。
